...
uint16_t Read_Keys(void)
{
	...
	val |= (HAL_GPIO_ReadPin(GPIOB, BT_11) == GPIO_PIN_RESET) ? (1 << 10) : 0;
...
void HC595_Send(uint16_t data)
{
    ...
    for (int i = 7; i >= 0; i--)
    {
        uint8_t bit1 = (data1 >> i) & 0x01;
        uint8_t bit2 = (data2 >> i) & 0x01;

        //同时设置两个 DS 引脚
        HAL_GPIO_WritePin(DS1_PORT, DS1_PIN, bit1 ? GPIO_PIN_SET : GPIO_PIN_RESET);
        HAL_GPIO_WritePin(DS2_PORT, DS2_PIN, bit2 ? GPIO_PIN_SET : GPIO_PIN_RESET);
